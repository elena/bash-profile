...
# http://andrewray.me/bash-prompt-builder/index.html#git=1&color-git=4&git-prefix=1&color-git-prefix=4&git-ahead=1&color-git-ahead=6b&git-modified=1&color-git-modified=4&git-conflicted=1&color-git-conflicted=1&git-revno=1&color-git-revno=4&git-bisect=1&color-git-bisect=2&option-submodule=1&color-option-submodule=2&git-ontag=1&color-git-ontag=4&svn=1&color-svn=6&svn-modified=1&color-svn-modified=6&svn-revno=1&color-svn-revno=6&option-modified=%E2%9C%8E&option-conflict=%E2%98%A2&color-option-conflict=3&max-conflicted-files=2&option-nobranch=no%20branch&color-option-nobranch=1&bisecting-text=%E2%9C%84&submodule-text=submodule%20
...
dvcs_function="
    ...
    if [ -n \"\$_git_dir\" ]; then
        ...
        if [ -z \"\$gitBranch\" ]; then
            ...
            if [ -z \"\$bisect\" ]; then
            noBranch=True
            gitBranch=\"\\[\$COLOR_RED\\]\$NOBRANCH_TEXT\\[\$COLOR_BLUE\\]\"
        else
            bisecting=\"\\[\$COLOR_GREEN\\]\$BISECTING_TEXT:\"\$bisect\"\\[\$COLOR_BLUE\\]\"
            gitBranch=\"\"
        fi
        fi
        ...
        if [ -n \"\$gitChange\" ]; then
            gitChange=\"\\[\$COLOR_BLUE\\] \\[`tput sc`\\]  \\[`tput rc`\\]\\[\$DELTA_CHAR\\] \"
        fi
        ...
        if is_submodule; then
            prompt=\$prompt\"\\[\$COLOR_MAGENTA\\]\$SUBMODULE_TEXT\\[\$COLOR_BLUE\\]\"
        fi

        prefix=\"\\[\$COLOR_BLUE\\]git:\\[\$COLOR_BLUE\\]\"
        ...
        if [ -n \"\$tag\" ]; then
            prompt=\"\$prompt\\[\$COLOR_BLUE\\] \\\"\$tag\\\"\\[\$COLOR_BLUE\\]\"
        fi
        prompt=\$prompt\"\$gitChange\\[\$COLOR_BLUE\\])\\[\$COLOR_RESET\\]\"
    ...
    if [ -n \"\$_hg_dir\" ]; then
        ...
        if [ -z \"\$bisecting\" ]; then
            ...
            prompt=\"\$prompt\\[\$COLOR_GREEN\\]:\$BISECTING_TEXT:\"\$bisecting\"\\[\$COLOR_MAGENTA\\]\"
        fi
        prompt=\$prompt\"\$hgChange\"
        patches=\${promptOptions[2]:1}
        if [ -n \"\$patches\" ];then
            prompt=\$prompt\"\\[\$COLOR_GREEN\\] [\$patches]\\[\$COLOR_MAGENTA\\]\"
        fi
        prompt=\$prompt\")\"
...
#PS1="\e]2;\w\a\u:\W \t\$(${dvcs_function})\[$COLOR_RESET\]\$ "
PS1="u:\W \t\$(${dvcs_function})\[$COLOR_RESET\]\$ "
